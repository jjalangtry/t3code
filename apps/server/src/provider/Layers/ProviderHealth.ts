...
import { Array, Effect, Layer, Option, Result, Stream } from "effect";
...
export function parseCursorAuthStatusFromOutput(result: CommandResult): {
  ...
  if (
    ...
    return {
      status: "warning",
      authStatus: "unknown",
      message: "Cursor CLI authentication status command is unavailable in this version.",
    };
  }
  ...
  if (parsedAuth.attemptedJsonParse) {
    return {
      status: "warning",
      authStatus: "unknown",
      message: "Could not verify Cursor authentication status from JSON output.",
    };
  }
  if (result.code === 0) {
    return { status: "ready", authStatus: "authenticated" };
